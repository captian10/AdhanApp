...
import { cancelExactAlarm, scheduleExactAlarm, canScheduleExactAlarms } from "../native/adhanAlarm";
import { getSoundPreference } from "./audioSettings";
...
const CACHED_CITY_KEY = "adhan_cached_city_name_v1"; // ✅ New Cache Key

// --- Location Helpers ---

export async function saveLocationPreference(mode: 'auto' | 'manual', cityData?: { lat: number, lng: number, name: string }) {
...
async function storeIds(ids: string[]) {
  try { await AsyncStorage.setItem(IDS_KEY, JSON.stringify(ids)); } catch {}
}
...
export async function refreshPrayerSchedule(options?: { daysAhead?: number, forceGps?: boolean }) {
  ...
  if (pref.mode === 'manual') {
    ...
    // If no last location, or user FORCED refresh, get fresh GPS (Slow)
    if (!loc) {
    ...
    if (cachedName && !options?.forceGps) {
        ...
        try {
            ...
            // If network fails, keep old name if exists
